...
public class P446_Arithmetic_Slices_2 {
    ...
    public int numberOfArithmeticSlices(int[] nums) {
        ...
        long ans = 0;
        Map<Integer, Integer>[] count = new Map[numsLength];
        for (int i = 0; i < nums.length; i++) {
            count[i] = new HashMap<>();
            for (int j = 0; j < i; j++) { // Can only look back for subsequence j..i
                long differenceLong = (long) nums[i] - (long) nums[j];
                if (differenceLong < Integer.MIN_VALUE || differenceLong > Integer.MAX_VALUE){
                    continue;
                }
